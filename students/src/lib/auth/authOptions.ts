...
const setCookie = async (cookieKey: string, value: string) => {
  ...
  cookiesStore.set(cookieKey, value, { maxAge:  30 * 24 * 60 * 60 });
};
...
export const authOptions: AuthOptions = {
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      if (account?.provider === 'google' && account.id_token) {
        ...
        setCookie('is-user-logged', 'false');

        throw new Error('Tài khoản chưa được cấp quyền đăng nhập bằng Google');
  ...
  debug: true, // Bật debug để xem log lỗi chi tiết
};
